// another program demonstrates how to plot various functions using the
...
int main()
{
    ...
    try
    {
        ...
        // NOTE: std::log() => log base e... ln()
        // shape that plots the function log() within the range
        // [0.000001, r_max) with its origin at orig using n_points line
        // segments scaled by  x_scale and y_scale
        Function f6 {log, 0.000001, r_max, orig, n_points, x_scale, y_scale};

        // shape that plots the function sin() within the range [r_min, r_max)
        // with its origin at orig using n_points line segments scaled by
        // x_scale and y_scale
        Function f7 {sin, r_min, r_max, orig, n_points, x_scale, y_scale};

        f7.set_color(Color::blue); // change Function f7's Color

        // NOTE: std::exp() => exponentiation... e^x
        // shape that plots the function exp() within the range [r_min, r_max)
        // with its origin at orig using n_points line segments scaled by
        // x_scale and y_scale
        Function f8 {exp, r_min, r_max, orig, n_points, x_scale, y_scale};

        f8.set_color(Color::green); // change Function f8's Color
